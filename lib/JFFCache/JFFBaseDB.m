...
//TODO what about varchar(512)???
static NSString* const create_records_ =
...
-(id)initWithDBName:( NSString* )db_name_
{
   ...
   if ( self )
   {
      NSString* const db_path_ = [ NSString documentsPathByAppendingPathComponent: db_name_ ];
          ...
          cacheName:( NSString* )cache_name_
{
   ...
   if ( self )
   {
      ...
      _db = [ [ JFFSQLiteDB alloc ] initWithDBName: db_name_ ];
      [ _db execQuery: create_records_ ];
   }
...
-(NSData*)dataForKey:( id )key_ lastUpdateTime:( NSDate** )date_
{
   ...
   if ( [ self.db prepareQuery: query_ statement: &statement_ ] )
   {
      if ( sqlite3_step( statement_ ) == SQLITE_ROW )
      {
         ...
         if ( date_ )
         {
            NSTimeInterval date_inetrval_ = sqlite3_column_double( statement_, date_index_ );
            *date_ = [ NSDate dateWithTimeIntervalSince1970: date_inetrval_ ];
         }
      }
      sqlite3_finalize( statement_ );
   }
